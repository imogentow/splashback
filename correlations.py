...
properties_all = np.vstack((accretion_xyz, M200m_xyz, energy_xyz,
                            Sgas_xyz, SDM_xyz, M14_xyz, flm.concentration,
...
list_good2 = np.intersect1d(
    np.intersect1d(np.intersect1d(np.where(np.isfinite(properties_all[5,:]))[0], 
                                  np.where(np.isfinite(properties_all[6,:]))[0]),
    np.intersect1d(np.where(np.isfinite(properties_all[7,:]))[0], np.where(np.isfinite(properties_all[8,:]))[0])),
    np.where(np.isfinite(properties_all[9,:]))[0])
...
labels = ["$\Gamma$", "$M_{\\rm{200m}}$", "$E_{\\rm{kin}}/E_{\\rm{therm}}$",
          "$S_{\\rm{gas}}$", "$S_{\\rm{DM}}$", "$\\rm{M14}$", "$c$", "$s$", "$a$",
...
lw = 2
ls = (0,(1,1))
ax.plot((5.5,9.5), (5.5,5.5), color=c, linewidth=lw, linestyle=ls)
ax.plot((5.5,5.5), (5.5,9.5), color=c, linewidth=lw, linestyle=ls)
ax.plot((9.45,9.45), (5.5,9.5), color=c, linewidth=lw, linestyle=ls)
ax.plot((5.5,9.5), (9.45,9.45), color=c, linewidth=lw, linestyle=ls)
